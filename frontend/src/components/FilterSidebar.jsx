...
import { Search, RotateCcw, Tag, Layers, SlidersHorizontal, X } from "lucide-react";
    ...
    <>
      ...
      <div className="flex flex-col gap-1.5">
        {UniqueCategory.map((item, i) => (
          <label key={i} className="flex items-center gap-2.5 cursor-pointer group" onClick={() => setCategory(item)}>
            <div
      ...
      <div className="space-y-3">
        ...
        <div className="relative h-5 flex items-center w-full overflow-hidden">
          ...
          />
          <input type="range" min="0" max="9999" step="100" value={priceRange[0]} onChange={handleMinChange}
            className="absolute w-full h-2 appearance-none bg-transparent cursor-pointer"
            style={{ accentColor: "#3d6b40" }} />
          <input type="range" min="0" max="9999" step="99" value={priceRange[1]} onChange={handleMaxChange}
            className="absolute w-full h-2 appearance-none bg-transparent cursor-pointer"
            style={{ accentColor: "#8fb97a" }} />
        </div>
        <div className="flex gap-2">
          <input type="number" min="0" max="5000" value={priceRange[0]} onChange={handleMinChange}
            placeholder="Min" className="w-full px-2 py-1.5 rounded-lg text-xs border text-center focus:outline-none"
            style={{ borderColor: "rgba(0,0,0,0.1)", color: "#2d2d2d" }} />
          <span className="text-gray-400 self-center">-</span>
          <input type="number" min="0" max="9999" value={priceRange[1]} onChange={handleMaxChange}
            placeholder="Max" className="w-full px-2 py-1.5 rounded-lg text-xs border text-center focus:outline-none"
            style={{ borderColor: "rgba(0,0,0,0.1)", color: "#2d2d2d" }} />
        </div>
      </div>
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setMobileOpen(false)}
                className="p-1.5 rounded-lg" style={{ background: "rgba(0,0,0,0.06)" }}>
            ...
            <button
              ...
              style={{ background: "linear-gradient(135deg, #2d4a2e, #3d6b40)", color: "#fff" }}
            >
              Apply Filters
            </button>
          </div>
