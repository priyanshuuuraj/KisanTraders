...
const Sidebar = () => {
  return (
    <>
      {/* DESKTOP Sidebar */}
      <div
        className="hidden md:flex fixed flex-col w-[240px] h-screen pt-20"
      ...
      <div
        className="md:hidden fixed bottom-0 left-0 right-0 z-30 flex items-center justify-around px-2 py-2 safe-area-bottom"
        style={{
          background: "rgba(26,46,26,0.97)",
          borderTop: "1px solid rgba(143,185,122,0.15)",
      ...
      >
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            className="flex flex-col items-center gap-1 px-2 py-1.5 rounded-xl transition-all duration-150 min-w-[52px]"
            style={({ isActive }) => ({
              color: isActive ? "#8fb97a" : "rgba(245,240,232,0.4)",
              background: isActive ? "rgba(143,185,122,0.12)" : "transparent",
            })}
          >
            {item.icon}
